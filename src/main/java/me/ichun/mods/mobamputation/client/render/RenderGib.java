...
public class RenderGib extends Render<EntityGib>
{
    ...
    public void doRender(EntityGib gib, double par2, double par4, double par6, float par8, float par9)
    {
        if(!gib.attached)
        {
            ...
            GlStateManager.color(1.0F, 1.0F, 1.0F, MathHelper.clamp_float(gib.groundTime >= MobAmputation.config.gibGroundTime ? 1.0F - (gib.groundTime - MobAmputation.config.gibGroundTime + par9) / 20F : 1.0F, 0F, 1F));
